    ...
    app.post("/api/volunteer/survey", function(req, res) {
        ...
        surveyModel.create(survey, function(err, survey) {
            if (err) {
                ...
                res.status(201).json({ success: "Resource successfully saved to database", id: survey._id});
                console.log(survey);
            }
        })
    });
            ...
            if (err) {
                ...
                survey.save(function(err, survey) {
                    if (err) {
                        ...
                        res.status(200).json({ success: "Survey updated successfully", survey: survey });
                        console.log(survey);
                    }
                });
            }
